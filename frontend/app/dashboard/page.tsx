...
const formatDueText = (value: string | null): string => {
...
const hoursUntil = (value: string | null): number | null => {
  if (!value) return null;
  const due = new Date(value).getTime();
  if (Number.isNaN(due)) return null;
  return Math.max(0, Math.ceil((due - Date.now()) / 3600000));
};
  ...
  if (error || !data) {
    return (
      <div style={{ padding: "32px 36px" }}>
        <p style={{ color: "var(--overdue)", marginBottom: 10 }}>{error || "Unable to load dashboard."}</p>
        <Link href="/dashboard/events" style={{ color: "var(--accent)", textDecoration: "none" }}>Go to My Events</Link>
      </div>
    );
  }
      ...
      <div className="fade-up fade-up-1" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 32 }}>
        <div>
          <h1 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.75rem", fontWeight: 800, color: "var(--text-1)", marginBottom: 4 }}>
            Good morning, {data.user?.name || "there"} 👋
          </h1>
          <p style={{ color: "var(--text-2)", fontSize: "0.875rem" }}>
            You have {data.stats.tasks_today} task(s) due today and {data.stats.overdue_events} overdue event(s).
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 24 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div className="fade-up fade-up-3 card" style={{ padding: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {events.length === 0 && <p style={{ margin: 0, color: "var(--text-3)", fontSize: "0.85rem" }}>No events yet. Create your first event.</p>}
              {events.map((ev) => (
                  ...
                  <div className="card-hover" style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 16px", background: "var(--surface-2)", borderRadius: 12, border: "1px solid var(--border)", cursor: "pointer" }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <span style={{ fontSize: "0.75rem", color: "var(--text-3)" }}>📅 {formatDate(ev.date)}</span>
          ...
          <div className="fade-up fade-up-4 card" style={{ padding: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {todayTasks.length === 0 && <p style={{ margin: 0, color: "var(--text-3)", fontSize: "0.85rem" }}>No tasks due today.</p>}
              {todayTasks.map((task) => (
                <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--surface-2)", borderRadius: 10, border: "1px solid var(--border)" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: "0.75rem", color: "var(--text-3)", margin: 0 }}>{task.event_name || "Unknown event"}</p>
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div className="fade-up fade-up-3 card" style={{ padding: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {deadlines.length === 0 && <p style={{ margin: 0, color: "var(--text-3)", fontSize: "0.8rem" }}>No upcoming deadlines in 48 hours.</p>}
              {deadlines.map((d) => {
                const hours = hoursUntil(d.due_date);
                return (
                  <div key={d.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "10px 12px", background: "var(--surface-2)", borderRadius: 10, border: `1px solid ${hours !== null && hours < 12 ? "rgba(255,107,107,0.3)" : "var(--border)"}` }}>
                    <div>
                      <p style={{ fontSize: "0.8rem", fontWeight: 500, color: "var(--text-1)", margin: 0 }}>{d.title}</p>
                      <p style={{ fontSize: "0.7rem", color: "var(--text-3)", margin: 0 }}>{d.event_name || "Unknown event"}</p>
                    </div>
                    <span style={{ fontSize: "0.7rem", color: hours !== null && hours < 12 ? "var(--overdue)" : "var(--at-risk)", fontWeight: 700, flexShrink: 0 }}>
                      {hours === null ? "-" : `${hours}h`}
                    </span>
                  </div>
                );
          ...
          <div className="fade-up fade-up-4 card" style={{ padding: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {activity.length === 0 && <p style={{ margin: 0, color: "var(--text-3)", fontSize: "0.8rem" }}>No recent activity.</p>}
              {activity.map((a, i) => (
                <div key={`${a.time}-${i}`} style={{ display: "flex", gap: 10, paddingBottom: i < activity.length - 1 ? 10 : 0, borderBottom: i < activity.length - 1 ? "1px solid var(--border)" : "none" }}>
